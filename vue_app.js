    ...
    methods: {
        ...
        async solvePuzzle() {
            ...
            try {
                ...
                const solution = await findSolution(this.columns, this.getSimplifiedState, this.validatePuzzleState, this.isGoalState, findEmptyPosition, PriorityQueue, this.debugState);
        ...
        async depthLimitedSearch(state, depthLimit, visited, nodesExplored, maxNodes) {
            ...
            while (stack.length > 0 && nodesExplored < maxNodes) {
                ...
                const moves = this.getSmartMoves(current.state, emptyPos)
                    .filter(move => {
                ...
                const sortedMoves = moves.map(move => ({
                    ...
                    heuristic: this.calculateHeuristic(move.apply(current.state)) +
                        ...
                        (this.moveLeadsToTarget(move.apply(current.state), move.move) ? -100 : 0) +
                        this.calculateBeadDistances(move.apply(current.state))
        ...
        calculateBeadDistances(state) {
            return state.reduce((sum, col, colIndex) =>
                sum + col.reduce((colSum, bead, rowIndex) =>
                    bead ? colSum + Math.abs(colIndex - this.getTargetColumnForBead(bead)) : colSum, 0), 0);
        },
